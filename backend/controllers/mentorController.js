...
const getMentor = asyncHandler(async (req, res) => {
  ...
  if (
    ...
    String(mentor.user._id) !== String(req.user._id)
  ) {
    return res.status(403).json({
      success: false,
      message: 'You cannot access this mentor profile'
    });
  }
  ...
  if (
    ...
    return res.status(403).json({
      success: false,
      message: 'You are not assigned to this mentor'
    });
  }
...
const getMentorStudents = asyncHandler(async (req, res) => {
  ...
  if (
    String(mentor.user) !== String(req.user._id)
  ) {
    return res.status(403).json({
      success: false,
      message: 'You can only view your assigned students'
    });
  }
